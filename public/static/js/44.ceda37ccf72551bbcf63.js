...
// CONCATENATED MODULE: ./node_modules/babel-loader/lib!./node_modules/vue-loader/lib/selector.js?type=script&index=0!./src/page/preAct/activeMode_7.vue
//
//
//
//
...
/* harmony default export */ var activeMode_7 = ({
  data() {
  ...
  methods: {
        ...
        if (_this.seachData.type === 'edit') {
          ...
          _this.$http.get(editUrl, URLCONFIG).then(res => {
            if (res.data.state === 0 && res.data.data) {
              ...
              formData.rule.rule.forEach(item => {
                if (item.type === '1') {
                  let data = {
                    money: item.money / 100,
                    type: item.type,
                    prize: item.prize / 100,
    ...
    doSubmit(type) {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          let ruleArr = JSON.parse(JSON.stringify(this.modeData.rule.rule));
          ruleArr.forEach(function (item) {
            if (item.type === '1') {
              delete item.prize1;
            } else {
              item.prize = item.prize1;
              delete item.prize1;
            }
    ...
    //优惠规则数字验证
    validatorTypeNumber(rule, value, callback) {
      let key = this.ruleKey;

      if (key !== '') {
        if (this.modeData.rule.rule[key].type === '1') {
    ...
    validatorMoneyNumber(rule, value, callback) {
      let key = this.ruleKey;

      if (key !== '') {
        if (this.modeData.rule.rule[key].type === '2') {
          this.validatorNumber.apply(this, arguments);
        } else {
          callback();
        }
    ...
    typeChange() {
      this.$refs.percentInput.forEach(item => {
        item.handleBlur();
      });
    }
...
/* scopeId */
var __vue_scopeId__ = "data-v-0111c28c"
...
/* harmony default export */ var preAct_activeMode_7 = __webpack_exports__["default"] = (Component.exports);
