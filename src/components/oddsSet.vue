<template>
    <div id="modelSet" v-loading="loading">
        ...
        <el-button type="primary" v-text="LANG['保 存'] || '保 存'" @click="doSave" style="position: absolute;right:10px;top:-36px" size="small"></el-button>
        <table cellspacing="0" cellpadding="0" border="0" class="el-table el-table__header el-table--border" style="width: 100%;">
            <thead>
                <tr>
                    <th colspan="1" rowspan="3" class="el-table_1_column_36 is-leaf">
                        <div class="cell"> <el-checkbox v-model="allChecked" @change="allCheckedOdd"></el-checkbox></div>
                    </th>
                    <th colspan="1" rowspan="3" class="el-table_1_column_36 is-leaf">
                    ...
                    <th colspan="1" rowspan="3" class="el-table_1_column_36 is-leaf" style="padding: 0 20px;">
                        <div class="cell" style="padding-top:46px;"  v-text="LANG['当前赔率'] || '当前赔率'"></div>
                            <input type="number"  v-model="inputone" placeholder="快调" class="el-input__inner el-input" style="margin-top:10px;display:block;"  @keyup.13="changeCol($event,'odds')" @blur="changeCol($event,'odds')" min="1">
                    </th>
                    <th colspan="1" rowspan="3" class="el-table_1_column_36 is-leaf">
                ...
                <tr>
                    <th colspan="1" rowspan="1" class="el-table_1_column_37_column_39_column_40 is-leaf">
                        <div class="cell">
                            <input type="number" min="2" v-model="inputtwo" placeholder="快调" class="el-input__inner el-input" style="margin-top:10px;display:block"  @keyup.13="changeCol($event,'min_bet')" @blur="changeCol($event,'min_bet')">
                        </div>
                    </th>
                    <th colspan="1" rowspan="1" class="el-table_1_column_37_column_39_column_41 is-leaf">
                        <div class="cell">
                            <input type="number" placeholder="快调" v-model="inputthree" class="el-input__inner el-input" style="margin-top:10px;display:block"  @keyup.13="changeCol($event,'one_bet')" @blur="changeCol($event,'one_bet')">
                        </div>
                    </th>
                    <th colspan="1" rowspan="1" class="el-table_1_column_37_column_39_column_42 is-leaf">
                        <div class="cell">
                            <input type="number" placeholder="快调" v-model="inputfour" class="el-input__inner el-input" style="margin-top:10px;display:block"  @keyup.13="changeCol($event,'one_term')" @blur="changeCol($event,'one_term')">
                        </div>
                    </th>
                    <th colspan="1" rowspan="1" class="el-table_1_column_37_column_39_column_43 is-leaf">
                        <div class="cell">
                            <input type="number" placeholder="快调" v-model="inputfive" class="el-input__inner el-input" style="margin-top:10px;display:block"  @keyup.13="changeCol($event,'one_max_money')" @blur="changeCol($event,'one_max_money')">
                        </div>
                    </th>
                </tr>
            </thead>
            <tr v-for="(item,index) in dataModel" v-if="dataModel.length">
                <td class="el-table_1_column_19">
                    <div class="cell"> <el-checkbox v-model="item.checked" @change="checkedOdd(index, item.checked)"></el-checkbox></div>
                </td>
                <td class="el-table_1_column_19">
                    <div class="cell" >
                ...
                <td class="el-table_1_column_19">
                    <div class="cell">
                        <div v-for="(sub,key) in item.sub_odds" style="clear: both;">
                            ...
                            <input type="number"  class="el-input__inner" v-model="sub.odds" @keyup.13="doEnter(index,'odds',key)" @blur="doEnter(index,'odds',key)" >
                        </div>
                ...
                <td class="el-table_1_column_19">
                    <div class="cell">{{item.min_odds}}</div>
                </td>
                <td class="el-table_1_column_19">
                    <div class="cell">{{item.max_odds}}</div>
                </td>
                <td class="el-table_1_column_19">
                    <div class="cell"> <el-checkbox v-model="item.status" :true-label="'enabled'" :false-label="'disabled'"></el-checkbox></div>
                </td>
                <td class="el-table_1_column_19">
                    <div class="cell">
                        <span @click="changeInput($event,index,'dzzx')">{{item.min_bet}}</span>
                        <input type="number"  class="el-input__inner" v-model="item.min_bet" @keyup.13="doEnter(index,'min_bet')" @blur="doEnter(index,'min_bet')">
                    </div>
                </td>
                <td class="el-table_1_column_19">
                    <div class="cell">
                        <span @click="changeInput($event,index,'dzzd')" >{{item.one_bet}}</span>
                        <input type="number"  class="el-input__inner" v-model="item.one_bet"  @keyup.13="doEnter(index,'one_bet')" @blur="doEnter(index,'one_bet')" >
                    </div>
                </td>
                <td class="el-table_1_column_19">
                    <div class="cell">
                        <span @click="changeInput($event,index,'dxzx')" >{{item.one_term}}</span>
                        <input type="number"  class="el-input__inner" v-model="item.one_term" @keyup.13="doEnter(index,'one_term')" @blur="doEnter(index,'one_term')">
                    </div>
                </td>
                <td class="el-table_1_column_19">
                    <div class="cell">
                        <span @click="changeInput($event,index,'dxzgpj')" >{{item.one_max_money}}</span>
                        <input type="number"  class="el-input__inner" v-model="item.one_max_money" @keyup.13="doEnter(index,'one_max_money')" @blur="doEnter(index,'one_max_money')">
                    </div>
                </td>
            </tr>
        ...
        <el-col :span="24" class="toolbar mt20" v-if="dataModel.length >0 "
            ...
            <el-pagination :current-page="currentPage" :page-sizes="[10,50,100,200,500]" :page-size="pageSize"
                           ...
                           :total="total" style="float:right" @current-change="doCurrentChange" @size-change="doSizePage">
            </el-pagination>
        </el-col>
    </div>
</template>
