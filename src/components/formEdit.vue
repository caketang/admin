<template>
    <div id="formEdit"  v-loading.body="formLoading" v-if="showEditFrom">
        <el-dialog :title="formTitel" v-model="formVisible.state" :size="size"  v-if="type != 'search' && type != 'twoCol' && !formLoading" @close="closeDialog('noReset')">
            <el-form :model="editForm" :rules="rules" ref="editForm" :label-width="labelWidth" class="pb10">
                <div v-for="(item,index) in formConfig" :key="index">
                    ...
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'text' || item.type == 'phone' || item.type == 'email' || item.type == 'url' || item.type == 'textarea' || item.type == 'ip' || item.type == 'account') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop" >
                        <el-input v-model="editForm[item.prop]" v-if="!(item.type == 'textarea') " :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)" @change="changeInput(item.prop)"></el-input>
                        <el-input v-model="editForm[item.prop]" type="textarea" v-if="item.type == 'textarea'" :rows="(parseInt(item.rows) || 5)"  :placeholder="LANG[item.placeholder] || item.placeholder"  @change="changeTextarea(editForm[item.prop])" :disabled="(item.disabled?true:false)"></el-input>
                        <span class="help_red" v-if="item.type == 'textarea'&& textareaTest && textareaTest.type && editForm[item.prop].length>textareaTest.length" >不能超过 <span>{{textareaTest.length}}</span>个字符最大长度</span>
                        <span class="help_gray" v-if="item.type == 'textarea'&& textareaTest && textareaTest.type && editForm[item.prop].length<=textareaTest.length" >当前还可以输入 <span>{{textareaTest.length-editForm[item.prop].length}}</span> 个字符</span>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'password') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-input v-model="editForm[item.prop]" type="password" v-if="!showPass" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)">
                            <el-button slot="append" @click="showpass" v-if="item.showpasswordname != false">
                                <i class="fa fa-eye" aria-hidden="true"></i>
                            </el-button>
                        </el-input>
                        <el-input v-model="editForm[item.prop]" type="text" v-if="showPass" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)">
                    ...
                    <!--匹配第三方支付和结算方式-->
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'passwordType')  && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                    ...
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'repeatPassword') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-input v-model="editForm[item.prop]" type="password" v-if="!showPass" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)">
                            <el-button slot="append" @click="showpass">
                                <i class="fa fa-eye" aria-hidden="true"></i>
                            </el-button>
                    ...
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'button') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop" >
                        <el-button @click="doButtonClick(item)">{{item.text}}</el-button> <span style="margin-left:10px;">{{item.value}}</span>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'number') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-input v-model="editForm[item.prop]" type="number" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)"></el-input>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'select') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-select v-model="editForm[item.prop]" :placeholder="LANG['请选择'] || '请选择'" @change="changeSelect(item.propVal,item.list,editForm[item.prop],item.relation,item.relationKey,item.linkArr,item.prop)" :disabled="item.disabled">
                            <el-option  v-for="(option,index) in item.list" :key="index" :label="option.label" :value="option.value"> </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'dateGroup') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :required="(item.required ?item.required: false)" >
                        <el-col :span="11">
                            <el-form-item :prop="item.prop[0].prop">
                                <datetimepicker v-model="editForm[item.prop[0].prop]" :datekey="item.prop[0].prop" :isEnd="false" @get-val="getDateVal" :showTime="item.showTime"></datetimepicker>
                            </el-form-item>
                        </el-col>
                        <el-col class="line textcenter" :span="2">-</el-col>
                        <el-col :span="11">
                            <el-form-item :prop="item.prop[1].prop">
                                <datetimepicker v-model="editForm[item.prop[1].prop]" :datekey="item.prop[1].prop" :isEnd="true" @get-val="getDateVal" :showTime="item.showTime"></datetimepicker>
                            </el-form-item>
                        </el-col>
                    </el-form-item>
                    <!--分割线-->
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'numberGroup') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)">
                        <el-col :span="11">
                            <el-form-item :prop="item.prop[0].prop">
                                <el-input v-model="editForm[item.prop[0].prop]"  type="number" size="small" class="intW"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="2">-</el-col>
                        <el-col :span="11">
                            <el-form-item :prop="item.prop[1].prop">
                                <el-input v-model="editForm[item.prop[1].prop]" type="number" size="small" class="intW" ></el-input>
                            </el-form-item>
                        </el-col>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'datePicker') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <mydatepick v-model="editForm[item.prop]"  @get-date="getDateval" :datekey="item.prop"></mydatepick>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'datePickernotime') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <datepick v-model="editForm[item.prop]"  @get-date="getDayVal" :datekey="item.prop"></datepick>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'dateWeek') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-date-picker type="week" :placeholder="LANG['选择周']  || '选择周'" v-model="editForm[item.prop]" @change="changeWeek(editForm[item.prop],item.propOne,item.propTwo)" ></el-date-picker>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'switch') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-switch v-model="editForm[item.prop]"></el-switch>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'checkGroup') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-checkbox :indeterminate="isIndeterminate[item.prop]" v-model="checkAll[item.prop]" @change="handleCheckAllChange($event,index,item.prop)" v-show="item.allBtnShow">{{LANG['全选']  || '全选'}}</el-checkbox>
                        <el-checkbox-group v-model="editForm[item.prop]" @change="handleCheckedCitiesChange($event,index,item.prop)">
                            <el-checkbox v-for="(box,index) in item.list" :key="index" :label="LANG[box] || box" name="type" class="mr10">{{item.Arr | formatcheckitem(box)}}</el-checkbox>
                        </el-checkbox-group>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'checkbox') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-checkbox v-model="editForm[item.prop]" >{{LANG[item.valLabel] || item.valLabel}}</el-checkbox>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'radioGroup') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                        <el-radio-group v-model="editForm[item.prop]"  @change="changeRadio(item.checkIndex,item.arr,editForm[item.prop],item.propkey)">
                            <el-radio v-for="(rad,index) in item.list" :key="index" :label="LANG[rad.value] || rad.value" >{{rad.label}}</el-radio>
                        </el-radio-group>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'dateTimeGroup') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :required="(item.required ?item.required: false)" >
                        <el-col :span="11">
                            <el-form-item :prop="item.prop[0].prop">
                                <datetimepicker v-model="editForm[item.prop[0].prop]"  :datekey="item.prop[0].prop" :isEnd="false" @get-val="getDateVal" :showTime="item.showTime"></datetimepicker>
                            </el-form-item>
                    ...
                    <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type =='markdown') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
        ...
        <el-col  :span="24" v-if="formConfig[0].type == 'quickDate' && !formLoading" style="margin-bottom: 10px;">
            <quickDate :reSet="reSet" @get-day="getDay" :defaultId=" formConfig[0].defaultId " v-if="formConfig[0].defaultId" :showTimes="formConfig[0].showTimes"></quickDate>
            <quickDate :reSet="reSet" @get-day="getDay" v-else :showTimes="formConfig[0].showTimes"></quickDate>
        </el-col>
        <el-form :model="editForm" :rules="rules" ref="editForm" :label-width="labelWidth" v-if="type == 'search' && !formLoading" :inline="true" class="search">
            <el-form-item  v-for="(item,index) in formConfig" :key="index" >
                <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'text' || item.type == 'phone' || item.type == 'email' || item.type == 'url' || item.type == 'ip' || item.type == 'account' || item.type == 'textarea'" :prop="item.prop">
                    <el-input v-model="editForm[item.prop]" v-if="item.type != 'textarea' && item.style" size="small" class="intW" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)" style="width: 250px !important;"></el-input>
                    <el-input v-model="editForm[item.prop]" class="intW" v-if="item.type != 'textarea' && item.style == undefined" size="small" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)" :class="{width150: item.width150}"></el-input>
                    <el-input v-model="editForm[item.prop]" type="textarea" v-if="item.type == 'textarea'" :rows="(item.rows || 1)"  size="small" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)" :style="{ width: item.width + 'px' }"></el-input>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'number'" :prop="item.prop">
                    <el-input v-model="editForm[item.prop]" type="number"  class="intW" size="small" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)"></el-input>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type =='select') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)"  :prop="item.prop">
                    <el-select v-model="editForm[item.prop]" :placeholder="LANG['请选择'] || '请选择'" class="intW" size="small" @change="changeSelect(item.propVal,item.list,editForm[item.prop],item.relation,item.relationKey,item.linkArr,item.prop)" :filterable="item.filterable" :disabled="item.disabled">
                        <el-option  v-for="(option,index) in item.list" :key="index" :label="option.label" :value="option.value"> </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type =='selectString') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                    <el-select v-model="editForm[item.prop]" :placeholder="LANG['请选择'] || '请选择'" class="intW" size="small" @change="changeSelect(item.propVal,item.list,editForm[item.prop],item.relation,item.relationKey,item.linkArr,item.prop)" :filterable="item.filterable" :disabled="item.disabled">
                        <el-option  v-for="(option,index) in item.list" :key="index" :label="option.label" :value="option.value"> </el-option>
                    </el-select>
                    <el-input v-model="editForm[item.propInput]" type="text" class="intW" size="small" style="width:120px;" :placeholder="LANG[item.Inputplaceholder] || item.Inputplaceholder" :disabled="(item.disabled?true:false)"></el-input>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'dateGroup'">
                    <el-col :span="11">
                        <el-form-item :prop="item.prop[0].prop">
                            <datetimepicker v-model="editForm[item.prop[0].prop]" :datekey="item.prop[0].prop" :isEnd="false" :showTime="item.showTime"></datetimepicker>
                        </el-form-item>
                    </el-col>
                    <el-col class="line textcenter" :span="2">-</el-col>
                    <el-col :span="11">
                        <el-form-item :prop="item.prop[1].prop">
                            <datetimepicker v-model="editForm[item.prop[1].prop]"  :datekey="item.prop[1].prop" :isEnd="true" :showTime="item.showTime"></datetimepicker>
                        </el-form-item>
                    </el-col>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'dateTimeGroup') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :required="(item.required ?item.required: false)" >
                ...
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'dateWeek') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                    <el-date-picker type="week" format="yyyy 第 WW 周" :placeholder="LANG['选择日期']  || '选择日期'" v-model="editForm[item.prop]" @change="changeWeek(editForm[item.prop],item.propOne,item.propTwo)" ></el-date-picker>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'switch'" :prop="item.prop">
                    <el-switch v-model="editForm[item.prop]"></el-switch>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'checkGroup'" :prop="item.prop">
                    <el-checkbox :indeterminate="isIndeterminate[item.prop]" v-model="checkAll[item.prop]" @change="handleCheckAllChange($event,index,item.prop)" v-show="item.allBtnShow" class="fl mr10">{{LANG['全选'] || '全选'}}<span class="ml10">|</span></el-checkbox>
                    <el-checkbox-group v-model="editForm[item.prop]" @change="handleCheckedCitiesChange($event,index,item.prop)" class="fl">
                        <el-checkbox v-for="(box,index) in item.list" :key="index" :label="LANG[box] || box" name="type" class="mr10">{{item.Arr | formatcheckitem(box)}}</el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
                ...
                <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'radioGroup'" :prop="item.prop">\
        ...
        <el-col style="text-align: right;margin-top: -10px;"  v-if="type == 'search' && !formLoading">
            <el-button type="primary" size="small" @click="addNew" class="tRight addBut" v-if="showAdd">{{addText}}</el-button>
        </el-col>
        <el-dialog :title="formTitel" v-model="formVisible.state"  v-if="type=='twoCol' && !formLoading" :size="large" @close="closeDialog">
            <el-form :model="editForm" :rules="rules" ref="editForm" :label-width="labelWidth" >
                <el-col v-for="(item,index) in formConfig" :key="index" :span="(!(item.type == 'dateGroup' || item.type == 'checkGroup' || item.type == 'radioGroup' || item.type =='upload' || item.type =='markdown')?12:24)">
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'text' || item.type == 'phone' || item.type == 'email' || item.type == 'url' || item.type == 'account' || item.type == 'textarea'" :prop="item.prop">
                        <el-input v-model="editForm[item.prop]" v-if="!(item.type == 'textarea')" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)"></el-input>
                        <el-input v-model="editForm[item.prop]" type="textarea" v-if="item.type == 'textarea'" :rows="(parseInt(item.rows) || 5)" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)"></el-input>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'password'" :prop="item.prop">
                        <el-input v-model="editForm[item.prop]" type="password" v-if="!showPass" :disabled="(item.disabled?true:false)"><el-button slot="append" class="fa fa-eye" aria-hidden="true" v-model="showPass" :placeholder="LANG[item.placeholder] || item.placeholder"></el-button></el-input>
                        <el-input v-model="editForm[item.prop]" type="text" v-if="showPass" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)"><el-button slot="append" class="fa fa-eye-slash" aria-hidden="true" ></el-button></el-input>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'repeatPassword'" :prop="item.prop">
                        <el-input v-model="editForm[item.prop]" type="password" v-if="!showPass" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)"></el-input>
                        <el-input v-model="editForm[item.prop]" type="text" v-if="showPass" :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)"></el-input>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'number'" :prop="item.prop">
                        <el-input v-model="editForm[item.prop]" type="number"  :placeholder="LANG[item.placeholder] || item.placeholder" :disabled="(item.disabled?true:false)"></el-input>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'select'" :prop="item.prop">
                        <el-select v-model="editForm[item.prop]" :placeholder="LANG['请选择'] || '请选择'" @change="changeSelect(item.propVal,item.list,editForm[item.prop],item.relation,item.relationKey,item.linkArr,item.prop)" :disabled="item.disabled">
                            <el-option  v-for="(option,index) in item.list" :key="index" :label="option.label" :value="option.value"> </el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'datePicker'" :prop="item.prop">
                        <mydatepick v-model="editForm[item.prop]"  @get-date="getDateval" :datekey="item.prop"></mydatepick>
                        <!--<el-date-picker type="date" :placeholder="LANG['选择日期'] || '选择日期'" v-model="editForm[item.prop]" ></el-date-picker>-->
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'dateWeek'" :prop="item.prop">
                    ...
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'checkGroup'" :prop="item.prop">
                        <el-checkbox :indeterminate="isIndeterminate[item.prop]" v-model="checkAll[item.prop]" @change="handleCheckAllChange($event,index,item.prop)" v-show="item.allBtnShow">{{LANG['全选'] || '全选'}}</el-checkbox>
                        <el-checkbox-group v-model="editForm[item.prop]" @change="handleCheckedCitiesChange($event,index,item.prop)">
                            <el-checkbox v-for="(box,index) in item.list" :key="index" :label="LANG[box] || box" name="type">{{item.Arr | formatcheckitem(box)}}</el-checkbox>
                        </el-checkbox-group>
                    </el-form-item>
                    <el-form-item :label="LANG[item.label] || item.label" v-if="item.type == 'radioGroup'" :prop="item.prop">
                        <el-radio-group v-model="editForm[item.prop]">
        ...
        <el-form :model="editForm" :rules="rules" ref="editForm" :label-width="labelWidth" v-if="type === 'pageForm'">
            <div v-for="(item,index) in formConfig" :key="index">
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'text' || item.type == 'phone' || item.type == 'email' || item.type == 'url' || item.type == 'textarea' || item.type == 'ip') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop" >
                    <el-input v-model="editForm[item.prop]" v-if="!(item.type == 'textarea')"></el-input>
                    <el-input v-model="editForm[item.prop]" type="textarea" v-if="item.type == 'textarea'" :rows="(parseInt(item.rows) || 5)"  @change="changeUrl(item.prop, item.domainType)"></el-input>
                ...
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'button') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop" >
                    ...
                    <img :src="editForm[item.prop] " alt="" v-if="editForm[item.prop]" style="margin-left: 10px;vertical-align: top;" height="40px">
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'number') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                ...
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'dateGroup') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" required >
                    <el-col :span="11">
                        <el-form-item :prop="item.prop[0].prop">
                            <datetimepicker v-model="editForm[item.prop[0].prop]"  :datekey="item.prop[0].prop" :isEnd="false" :showTime="item.showTime"></datetimepicker>
                        </el-form-item>
                    ...
                    <el-col :span="11">
                        <el-form-item :prop="item.prop[1].prop">
                            <datetimepicker v-model="editForm[item.prop[1].prop]"  :datekey="item.prop[1].prop" :isEnd="false" :showTime="item.showTime"></datetimepicker>
                        </el-form-item>
                    </el-col>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type == 'numberGroup') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)">
                ...
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type =='markdown') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)">
                    <markdownEdit v-model="editForm[item.prop]" :id="item.prop" :config="item.config" @getContentLength="getconLength"></markdownEdit>
                </el-form-item>
                <el-form-item :label="LANG[item.label] || item.label" v-if="(item.type =='upload') && (item.ifKey ? (editForm[item.ifKey] == item.ifVal) :true)" :prop="item.prop">
                    <upload  @response="doSaveFile" :keys="item.prop" :folder="item.folder" :isInit="imgInit" :arrList="item.value" :fileNum="item.fileNum"></upload>
...
<style>
    #formEdit .search{min-width: 860px;}
    #formEdit .el-date-editor.el-input{width:120px; height:30px;}
    #formEdit .el-form-item:first-child{margin-right:0;}
    #formEdit .el-input__inner{height:30px;}
    #formEdit .formbtn{float:left}
    #formEdit .el-form-item__content .el-date-editor .el-icon-date{padding-top:3px;}
    #formEdit .el-dialog__wrapper .el-dialog .el-dialog__body .el-form{overflow: inherit;}
    #formEdit .el-dialog__wrapper .el-dialog .el-dialog__body .el-form .el-select{width:100%;}
    #formEdit .el-checkbox+.el-checkbox{ margin-left:0px;}
    #formEdit .el-dialog__wrapper .el-dialog .el-dialog__body .el-checkbox{margin-right:15px;}
    #formEdit .el-radio__label {white-space: initial;}
    #formEdit .el-radio-group {
        width: 100%;
    }
    #formEdit .width150{width: 155px !important;}
</style>
