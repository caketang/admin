      ...
      <el-col>
            <tablegrid
                ...
                <tr :span="12" class="sumdiv ml20" slot="other" v-if="sumShow">
                    <!--<td colspan="15">-->
                        <!--<div class="cell">-->
                            <!--<span class="font16 ml10">{{LANG['优惠金额小计'] || '优惠金额小计'}} </span>:<span> {{allData.subTotalMoney.coupon_money | formatMoney}}</span>-->
                            <!--<span class="font16 ml10">{{LANG['优惠金额总计'] || '优惠金额总计'}} </span>:<span> {{allData.totalMoney.coupon_money | formatMoney}}</span>-->
                        <!--</div>-->
                    <!--</td>-->
                </tr>
            </tablegrid>
...
//			},
      ...
      //修改未处理的取款条件
			...
			doAllAdopt(rows) {
				...
				this.$.autoAjax('put',URL.api + ROUTES.PUT.active.apply.status, {"id": ids.toString(), "status": "pass"}, {
					ok: (res) => {
			...
			doAllRefuse(rows) {
				...
				this.$.autoAjax('put',URL.api + ROUTES.PUT.active.apply.status,{"id": ids.toString(), "status": "rejected"}, {
					ok: (res) => {
						if (res.state === 0 && res.data) {
			...
			submitAllow(row) {
				...
				this.$.autoAjax('put',URL.api + ROUTES.PUT.active.apply.status,{id: row.id, status: 'pass', user_id: parseInt(row.user_id), user_name: row.user_name, memo:row.memo}, {
					ok: (res) => {
						if (res.state === 0 && res.data) {
			...
			submitRefuse(row) {
				...
				this.$.autoAjax('put',URL.api + ROUTES.PUT.active.apply.status, {id: row.id, status: 'rejected', user_id: parseInt(row.user_id), user_name: row.user_name, memo:row.memo}, {
					ok: (res) => {
						if (res.state === 0 && res.data) {
