       ...
       <el-row>
           <el-col :span="24">
               <div class="echarMode mt20" style="background: #fff;">
                   <div class="w50 fl">{{LANG['交易数据'] || '交易数据'}}</div>
                   <div class="w50 fl tRight" v-if="hide">
                       <el-button  size="mini" @click="sevenDaysMenber(7)" class="ml300" :class="{'btn-active' :(day.memberActive)}">{{LANG['最近7天'] || '最近7天'}}
                       </el-button>
                       <el-button  size="mini" @click="sevenDaysMenber(30)" :class="{'btn-active' :!(day.memberActive)}">{{LANG['最近30天'] || '最近30天'}}</el-button>
                   </div>
         ...
         <el-row type="flex" :gutter="10" justify="center" class="circle">
            <el-col :span="5">
                <div class="box">
                     <span class="change">转化率80%</span>
                    <span class="total">交易额10000</span>
                    <span class="pay">线下支付</span>
                </div>
            </el-col>
            <el-col :span="5">
                <div class="box">
    ...
    export default{
        ...
        methods: {
			...
			initStat() {
				...
				_this.$.autoAjax('get', URL.api + ROUTES.GET.stat.today, '', {
					ok: (res) => {
					...
					this.$.autoAjax('get', URL.api + ROUTES.GET.stat.member.$(parseInt(val)), '', {
						ok: (res) => {
							if (res.state == 0 && res.data) {
								...
								res.data.day = ['1h', '2h', '3h', '4h', '5h', '6h', '7h', '8h', '9h', '10h', '11h', '12h', '13h', '14h', '15h', '16h', '17h', '18h', '19h',
									'20h', '21h', '22h', '23h', '24h'];
								_this.state.memberShow = true;
								for (let i = 0; i < 11; i++) {
									res.data.deposit.push(parseInt(Math.random() * 12000 + 5000));
									res.data.newly.push(parseInt(Math.random() * 12000 + 4000));
									res.data.orders.push(parseInt(Math.random() * 12000 + 3000));
									res.data.total.push(parseInt(Math.random() * 12000 + 3000));
