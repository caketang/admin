<template>
    <div id="onLineReceipts" class="page clearfix" v-loading="loading" >
        <formEdit :formVisible="formVisible" :formConfig="searchConfig" :updateKeys="updateKeys" :type="type" :labelWidth="labelWidth" @do-query="doQuery" :isEdit="isEdit" @reset-form="resetForm" :formType="formType" :showAdd="false"></formEdit>
        ...
        <el-col>
            <tablegrid
                ...
                <tr :span="12" class="sumdiv ml20" slot="other" v-if="sumShow">
                    <td colspan="15">
                        <div class="cell">
                            <span class="font16 ">{{LANG['金额小计'] || '金额小计'}} </span>:<span> {{allData.page_amount_sum | formatMoney}}</span>
                            <span class="font16 ml10">{{LANG['优惠小计'] || '优惠小计'}} </span>:<span> {{allData.page_coupon_sum | formatMoney}}</span>
                            <span class="font16 ml10">{{LANG['金额总计'] || '金额总计'}} </span>:<span> {{allData.total_amount_sum | formatMoney}}</span>
                            <span class="font16 ml10">{{LANG['优惠总计'] || '优惠总计'}} </span>:<span> {{allData.total_coupon_sum | formatMoney}}</span>
                ...
                <el-col :span="24" class="xsDetails">
                    <el-row :gutter="30">
                        <el-col :span="24" class="dialogList">
                            <div>
                                <h3>入款详情:</h3>
                                <el-col :span="12" >交易订单号:<span>{{dialogText.trade_no}}</span></el-col><el-col :span="12">是否首存：<span>{{dialogText.is_new | isNew}}</span></el-col>
                                <el-col :span="12">用户名： <span>{{dialogText.user_name}}</span></el-col><el-col :span="12">会员等级：<span>{{dialogText.ranting}}</span></el-col>
                                <el-col :span="12">真实姓名：<span>{{dialogText.true_name}}</span></el-col><el-col :span="12">用户注册时间：<span>{{dialogText.register_time}}</span></el-col>
                                <el-col :span="12">上级代理：<span>{{dialogText.agent_name}}</span></el-col><el-col :span="12">已入款次数/金额：<span>{{dialogText.total_deposit_times}}/{{dialogText.total_deposit_money/100}}</span></el-col>
                            </div>
                        </el-col>
                        <el-col :span="24" class="dialogList mt10"><div>
                            <h3>支付渠道:</h3>
                            <el-col :span="12">存款金额：<span>{{dialogText.money/100}}</span></el-col><el-col :span="12">支付接口名称：<span>{{dialogText.pay_no}}</span></el-col>
                            <el-col :span="12">支付渠道： <span>{{dialogText.channel_name |getChannel }}</span></el-col><el-col :span="12">外部交易号：<span>{{dialogText.trade_no}}</span></el-col>
                            <el-col :span="12">存款IP与地区：<span>{{dialogText.ip}}</span></el-col><el-col :span="12">存款日期：<span>{{dialogText.recharge_time}}</span></el-col>
                        </div></el-col>
                        <el-col :span="24" class="dialogList mt10"><div>
                            <h3>优惠:</h3>
                            <el-col :span="12">优惠金额：<span>{{dialogText.coupon_money/100}}</span></el-col><el-col :span="12">取款要求：<span>{{dialogText.withdraw_bet/100}}</span></el-col>
                        ...
                        <el-col :span="24" class="tCent el-dialog__footer" style="margin:20px 0; padding: 0; border-top: 0;"><div><el-button @click="dialogVisible = false">关 闭</el-button></div></el-col>
                    </el-row>
                </el-col>
        </el-dialog>
        </el-col>
...
//                        "list":ARRAYS.registrationSourceTwo, "filterable": true},
                    ...
                    {"type":"dateGroup","label":"提交时间","prop":[{"prop":"date_from","value":"","label":"开始时间"},{"prop":"date_to","value":"","label":"结束时间"}]},
                    // 优惠搜索
                    {
                    ...
                    // ID8
                    {"type":"numberGroup","label":"交易金额","prop":[{"prop":"money_from","value":""},{"prop":"money_to","value":""}]},
                    {"prop":"admin_user","value":"","type":"text","label":"操作者"},
                    {"prop":"ranting","value":[],"label":"会员等级","sReset":false,"checkAll":true,"allBtnShow":true,"type":"checkGroup","list":[],"Arr":[],"ifVal":"1","ifKey":"send_type"},
...
<style scoped>
    #onLineReceipts .page{position: relative}
    #onLineReceipts .details .el-dialog__body{height:450px;}
    #onLineReceipts .xsDetails{height: auto;}
    #onLineReceipts .dialogList{border:1px solid #8492A6;width: 90%;margin-left: 5.5%;padding:10px 0;}
    #onLineReceipts .borNone{border:none;}
    #onLineReceipts .dialogList .el-col-12{line-height:30px;}
    ...
    #onLineReceipts .sumdiv{position:relative;}
</style>
