<template>
    <div id="childRoleAccount" v-loading="loading" class="clearfix">
        <el-col class="pb">
            <el-button type="primary" class="addManage" size="small" @click="doAdd" v-text="LANG['新增角色'] || '新增角色'" ></el-button></el-col>
        <el-col>
            <tablegrid
        ...
        <el-col>
            <!--编辑界面-->
            <el-dialog :title="editFormTtile" v-model="editFormVisible" class="fromTitle" size="small">
                <el-form :model="editForm" label-width="100px" :rules="editFormRules" ref="editForm">
                    <el-form-item :label="LANG['角色名称'] || '角色名称'" prop="role">
                        <el-input v-model="editForm.role" auto-complete="off"></el-input>
                    </el-form-item>
                     <el-form-item :label="LANG['菜单权限'] || '菜单权限'" prop="">
                         <el-tree
                    ...
                    <el-form-item :label="LANG['会员权限'] || '会员权限'" >
                        <checkgroup
                            :checkGrounpList="checkGrounpList">
                        </checkgroup>
                    </el-form-item>
                    <el-form-item :label="LANG['导出权限'] || '导出权限'" >
                        <checkgroup
                                :checkGrounpList="checkExportList">
                        </checkgroup>
                    </el-form-item>
                </el-form>
                <div slot="footer" class="dialog-footer">
                    <el-button @click.native="resetForm">{{LANG['取消'] || '取消'}}</el-button>
                    <el-button type="primary" @click.native="saveFrom">{{LANG[btnEditText] || btnEditText}}</el-button>
                </div>
            </el-dialog>
    ...
    export default{
            ...
            return{
                ...
                checkGrounpList: [
                    ...
                    },
                    {
                        ...
                        checkedChildren: [],
                        childrenStatus: [
                            {cn_name: "停号", en_name: "account_disabled", status: false},
                            {cn_name: "封号", en_name: "account_forbidden", status: false},
                            {cn_name: "黑名单", en_name: "account_black", status: false},
                            {cn_name: "踢出在线", en_name: "account_kick", status: false},
                            {cn_name: "自我限制", en_name: "account_limit", status: false}
                        ]
                    },
                    {
                        name: "银行信息",
                        ...
                        checkedChildren: [],
                        childrenStatus: [
                            {cn_name: "银行卡号和开户信息", en_name: "bank_card", status: false}
                        ]
                    },
                    {
                        name: "联系信息",
                        ...
                        checkedChildren: [],
                        childrenStatus: [
                            {cn_name: "qq", en_name: "qq", status: false},
                            {cn_name: "手机号码", en_name: "mobile", status: false},
                            {cn_name: "邮箱", en_name: "email", status: false},
                            {cn_name: "微信", en_name: "weixin", status: false},
                            {cn_name: "skype", en_name: "skype", status: false},
                            {cn_name: "安全问题与答案", en_name: "answer", status: false},
                            {cn_name: "出生日期", en_name: "birth", status: false},
                            {cn_name: "出生地", en_name: "address", status: false},
                            {cn_name: "性别", en_name: "gender", status: false},
                            {cn_name: "邮编与国籍", en_name: "nationality", status: false},
                            {cn_name: "货币与语言", en_name: "language", status: false},
                            {cn_name: "身份证号码", en_name: "idcard", status: false}
                        ]
                    }
                ],
                // 导出权限
                checkExportList: [
                    {
                        ...
                        checkAll: false,
                        // isIndeterminate: false,
                        children: ["会员导出","代理导出","有效用户导出"],
                        checkedChildren: [],
                        childrenStatus: [
                            {cn_name: "会员导出", en_name: "member", status: false},
                            {cn_name: "代理导出", en_name: "agent", status: false},
                            {cn_name: "有效用户导出", en_name: "effective_user", status: false}
                            ]
                    }
                ]
            }
        ...
        methods: {
            init(){
                ...
                this.$.autoAjax('get',URL.api + ROUTES.GET.admin.subaccount.auth +'?id=0', '', {
                    ok: (res) => {
                        if(res.state === 0 && res.data){
                            let model = res.data.routes || [];
                ...
                // this.$http.get(URL.api + ROUTES.GET.admin.subaccount.auth +'?id=0',URLCONFIG).then((res)=>{
                ...
                switch (obj.fn) {
                    case "delete":
                        if(parseInt(obj.obj.id)>0 && parseInt(obj.obj.num) === 0){
                            ...
                            this.$.autoAjax('delete',URL.api + ROUTES.DELETE.sub.accounts.role.$(parseInt(obj.obj.id)), {
                                ok: (res) => {
                                    if(res.state === 0 && res.data){
                                        _this.updated = true;
                            ...
                            // this.$http.delete(URL.api + ROUTES.DELETE.sub.accounts.role.$(parseInt(obj.obj.id)),URLCONFIG).then((res)=>{
                            //     if(res.data.state === 0 && res.data.data){
                            //         _this.updated = true;
                            //         _this.$message.success(LANG['恭喜您，角色删除成功！'] || '恭喜您，角色删除成功！');
                        ...
                        this.$.autoAjax('put',url, query,{
                            ok: (res) => {
                                if (res.state == 0 && res.data) {
                                    ...
                                    if(this.formType === 'add'){
                                        this.$message.error((res.message? res.message : '') + (LANG['新增角色失败'] || '新增角色失败'));
                                    }else {
                                        this.$message.error((res.message ? res.message : '') + (LANG['修改角色失败'] || '修改角色失败'));
                                    }
                                }
                                _this.initRoleList();
                                _this.loading = false;
            ...
            doEdit(row){
                ...
                this.$.autoAjax('get',this.tableUrl + "/" + row.id , '', {
                    ok: (res) => {
                        if (res.state == 0 && res.data) {
                            let model = res.data[0].auth || [];
                            editForm.role = res.data[0].role || "";
...
<style scoped>
    #childRoleAccount .addManage{float:right;}
    #childRoleAccount .treeChildContent .is-expanded  .el-tree-node__children .is-expanded .el-tree-node__children .el-tree-node .el-tree-node__content{width:18%;float:left;}
