<template>
	<div id="indexEcharts" class="chart w100">
		<el-row :gutter="20">
			...
			<el-col :span="4">
				<div class="indexData">
					<p style="color:#0fbed1;" class="num" :title="'今日活跃用户：'+active">{{active}}</p>
					<p class="textHide">{{LANG['今日活跃用户'] || '今日活跃用户'}}</p>
				</div>
			</el-col>
			<el-col :span="4">
				<div class="indexData ">
					<p style="color:#39bc30;" class="num" :title="'今日新增用户：'+newly">{{newly}}</p>
					<p class="textHide">{{LANG['今日新增用户'] || '今日新增用户'}}</p></div>
			</el-col>
			<el-col :span="4">
				<div class="indexData">
					<p style="color:#ff6360;" class="num" :title="'今日上线用户数：'+online">{{online}}</p>
					<p class="textHide">{{LANG['今日上线用户数'] || '今日上线用户数'}}</p>
				</div>
			</el-col>
			<el-col :span="4">
				<div class="indexData">
					<p style="color:#ff9269;" class="num">{{parseInt(deposite) > 0 ? parseInt(deposite) / 100 : 0}}</p>
					<p class="textHide">{{LANG['今日存款总金额'] || '今日存款总金额'}}</p>
				</div>
			</el-col>
			<el-col :span="4">
				<div class="indexData">
					<p style="color:#ffb72f;" class="num" :title="'今日总注单数：'+orders">{{orders}}</p>
					<p class="textHide">{{LANG['今日总注单数'] || '今日总注单数'}}</p>
				</div>
			</el-col>
			<el-col :span="4">
				<div class="indexData">
					<p style="color:#ab8def;" class="num">{{parseInt(amount) > 0 ? parseInt(amount) / 100 : 0}}</p>
