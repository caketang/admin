<template>
    <div id="lotteryOdds" class="clearfix" v-loading="loading">
        ...
        <el-col class="btngroup">
            <el-button v-for="(item,index) in lotteryList" :key="index" @click="changeGame(item,index)"
                       class="formCancel" style="margin-bottom: 10px;" size="small"
                       :class="{'el-button--primary':parseInt(item.id) == parseInt(nowLotteryId)}">{{item.name}}
            </el-button>
        </el-col>
        <el-col>
            <el-button type="text" v-for="(item,index) in playTypeList" :key="index" @click="changePlayType(item)"
                       class="formCancel" :class="{'el-button--primary':parseInt(item.id) == parseInt(nowPlayTypeId)}"
                       style="padding: 10px;">{{item.name}}
            </el-button>
        </el-col>
        <el-col>
            <oddsSet :tableUrl="tableUrl" :columnsUrl="columnsUrl" @save-data="saveOdds" :playTypeId="lotteryTypeId"
                     :updated="updated"></oddsSet>
        </el-col>
    </div>
</template>
<script>
...
<style scoped>
    #lotteryOdds .btngroup > .el-button {
        margin-right: 10px;
    }

    #lotteryOdds .btngroup > .el-button + .el-button {
        margin-left: 0;
    }
</style>
